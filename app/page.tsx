...
export default function Home() {
  ...
  const [photoName, setPhotoName] = useState("Falta fotografía real"); const [photoLoaded, setPhotoLoaded] = useState(false); const [notice, setNotice] = useState<string | null>(null); const [exporting, setExporting] = useState(false);
  ...
  const render = useCallback((output: FormatId, canvas: HTMLCanvasElement) => {
    const { width, height } = FORMATS[output]; canvas.width = width; canvas.height = height; const ctx = canvas.getContext("2d"); if (!ctx) return;
  ...
  useEffect(() => { if (canvasRef.current) render(format, canvasRef.current); }, [format, render]);
  ...
  const loadPhoto = (e: ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (!f || !f.type.startsWith("image/")) return; const reader = new FileReader(); reader.onload = () => { const img = new Image(); img.onload = () => { imageRef.current = img; setPhotoName(f.name); setPhotoLoaded(true); setNotice("Fotografía real vinculada a esta ficha."); if (canvasRef.current) render(format, canvasRef.current); }; img.src = String(reader.result); }; reader.readAsDataURL(f); };
